...
export const userRouter = createTRPCRouter({
    ...
    //         return ctx.db.team.findUnique({ where: { id: id.toString() }, include: { members: true, settings: true } })
    //     }),
    ...
    getSettings: protectedProcedure.query(({ ctx }) => {
        return ctx.db.user.findFirst({ where: { id: ctx.session.user.id } }).settings
    })
